...
def processed_data_file():
    ...
    ## Reverse Sort Raw Data
    test_data = sorted(test_data, key=lambda x: x["created_utc"], reverse=True)
    ## Save Test Data
...
def test_emoji_handling(processed_data_file):
    ...
    assert replace_filtered[-1]["text_tokenized"] == \
    ...
    assert strip_filtered[-1]["text_tokenized"] == \
